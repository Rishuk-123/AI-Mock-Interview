...
function InterviewRoom() {
  ...
  useEffect(() => {
    const fetchInterview = async () => {
      try {
        ...
        if (interviewData.status === "scheduled" && !startRequested.current) {
          startRequested.current = true;

          const startResponse = await api.post(`/interviews/${id}/start`);
  ...
  const handleSubmitAnswer = async () => {
    ...
    try {
      ...
      const evaluationResponse = await api.post(`/interviews/${id}/evaluate`, {
        questionIndex: currentQuestion,
      });
      ...
      toast.success(`AI Score: ${evaluation.score}/100`);
    ...
    return (
      <MainLayout>
        <div className="flex min-h-[400px] flex-col items-center justify-center gap-4">
          <p className="text-slate-600">No interview questions available.</p>
