...
export default function InterviewResults() {
  ...
  // Extract raw incoming question/answer structures
  const rawQuestions = location.state?.questions || savedSession?.questions || [];
  const rawAnswers = location.state?.answers || savedSession?.answers || [];

  // Normalize questions to a flat array of strings
  const questions = rawQuestions.map((q) =>
    typeof q === "string" ? q : q.question || ""
  );

  // Normalize answers to a flat array of strings (pulling from object if needed)
  const answers =
    rawAnswers.length > 0
      ? rawAnswers
      : rawQuestions.map((q) => (typeof q === "object" ? q.answer || "" : ""));
  ...
  // Preload evaluation if passed directly
            ...
            <div className="mt-6 grid grid-cols-1 gap-4 sm:grid-cols-3">
              ...
              <div className="rounded-xl border border-slate-200/80 bg-slate-50/50 p-4">
                ...
                <p className="mt-2 text-2xl font-extrabold text-emerald-600">
                  {validAnswersCount} / {questions.length}
                </p>
              </div>
              ...
              return (
                <div key={idx} className="rounded-2xl border border-slate-200/80 bg-white p-6 shadow-sm">
                  <div className="flex items-start justify-between gap-4">
                    ...
                    <p className="flex-1 text-sm font-bold text-slate-900">{qText}</p>
                  </div>
